...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):
    ...
    if candidate in [CAR.SANTA_FE, CAR.SANTA_FE_1]:
      ...
      ret.lateralTuning.indi.outerLoopGain = 3 #4.8
      ret.lateralTuning.indi.timeConstant = 1.08 #1.16
      ret.lateralTuning.indi.actuatorEffectiveness = 1.08 #1.13
      #ret.lateralTuning.pid.kf = 0.00005
      ret.mass = 1737. + STD_CARGO_KG
      ret.wheelbase = 2.7
      ret.steerRatio = 13.73  #Spec 13.73
      tire_stiffness_factor = 0.385
      ret.steerActuatorDelay = 0.45 #0.3
      ret.steerLimitTimer = 0.2 #0.8
      #ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kpBP = [[0.], [0.]]
      #ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.25], [0.05]]
    ...
    ret.longitudinalTuning.kpV = [1.2, 0.7, 0.4] #[1.2, 0.6, 0.2]
  ...
  def update(self, c, can_strings):
    ...
    if self.CC.longcontrol and self.CS.cruise_unavail and self.CP.carFingerprint != CAR.KIA_NIRO_EV:
      events.add(EventName.brakeUnavailable)
    if abs(ret.steeringAngle) > 110. and EventName.steerTempUnavailable not in events.events:
